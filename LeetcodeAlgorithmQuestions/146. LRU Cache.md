...
1. If the HashMap does not contain the key, there will be 2 conditions. First is that size of the cache has reached the capacity limit, we need to remove the oldest key and value from both the LinkedList and HashMap, which locates at the end of the LinkedList, before we add the new key-value pair into both HashMap and LinkedList. Second is that the size of the cache has not reached the capacity, then we simply add the key-value pair.  
2. Otherwise, if the key already exists in the cache, then we need to update the value of the cache and put the key-value pair to the top since it is recently used. (The size cannot exceeds the capacity in this case since we would have deleted any old keys if the capacity is reached). 
